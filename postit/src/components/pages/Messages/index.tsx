...
export class MessagesPage extends React.Component<any, IMessagePageState> {
  ...
  async fetchUserCards(){
    ...
    allMessages.forEach((m)=>{
      ...
      else if (m.receiverId !== this.props.userId && !users.includes(m.receiverId)){
    ...
    for(let i:number=0; i < users.length; i++){
      let user : User = await getUsersById(users[i]);
      this.setState({userCards: [...this.state.userCards, user]});
      ...
      <Container className='main-container'>
        <Row className='h-100'>
          <Col className='title-row message-panel' xs={4}>
            ...
            <Row>
              <MessageListContainer userCards={this.state.userCards} setSelectedUser={(user:User)=>{this.setSelectedUser(user)}}/>
            </Row>
          </Col>
